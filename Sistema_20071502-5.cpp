...
int main(){
	...
	do{
		...
		printf(" Selecione uma opcão por favor =>: ");
		scanf("%d", &op);
		switch(op){
			case 1:
				resp='s';
                ...
                system("cls");
                printf("Deseja cadastrar novo produto[S] - SIM [N] - NÃO: ");
...
void file(Cadastroproduto produto[MAX]){
	...
	if( arq == NULL ){
        perror("Não existe o arquivo obras.txt");
        getchar();
        exit(-1);
    }
    int cont;
...
void identificacao(){
	system("cls");
		...
		printf("NOME: ÁDRIAN HONORIO DE FARIA\n");
